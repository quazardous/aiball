...
- The Stop hook now primes its loop-state reads (busy-defer, AFK active, human-typing) from the timer's live snapshot over `loop.sock` instead of `existsSync` / `readFileSync` on the marker files. The `humanIsTyping`, `afkActive` and `readBusyDefer` helpers honor strict-IPC-read mode (set by `queryLoopState` on a successful UDS round-trip) and skip the file shadow. Fail-open is preserved : a missing socket or a timeout leaves strict mode off and the helpers fall back to their original file path (#840 Slice C1 / #766).
