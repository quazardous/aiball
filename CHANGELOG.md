...
- **Windows: upgrading needs the installer re-run once.** The packaged
  `.cmd` wrappers are gone — the Windows launchers are now generated by the
  installer and point straight at the Node entry points. Shims written by an
  earlier installer name the deleted files, so `aiball`, `claude-loop` and
  `aiball-mcp` stop resolving from the PATH until `install.ps1` runs again.
  The MCP server goes with them, which reads as an agent losing its aiball
  tools rather than as a broken launcher. `aiball check` now says so:
  its `installed commands` section runs each command and names the repair.
