...
- **Wait credit**: `continue_after_minutes` now spends an agent's credit on the
  ...
  or wontfix, and the commits it cites on a reply (`commits`), by diff size.
...
- **A waiting ticket hears when its blocker closes**: closing a ticket posts a
  ...
  instead of finding out at the next backlog pass. That relation is how an
  agent says its ticket waits on another, and the agent a ticket is assigned
  to may now set it even when it neither filed the ticket nor owns the project.
...
- **An agent's comment must carry a decision or say it is only a comment**: a
  ...
  tickets that nobody held. The same goes for a ticket an agent creates: it
  carries `then: plan`, or `comment_only: true` when it only sets down something
  to remember (`--plan` / `--comment-only` on `aiball ticket new`). Humans are
  exempt; `tickets.require_then` switches the rule off per project.
...
- Answering a question about one ticket no longer reads the whole board. Four
  ...
  handful they were asked about — the cost every accept, close and inbox refresh
  paid, growing with the corpus. Warm, that work went from 94ms to 3ms; cold,
  from 563ms to 394ms.
...
- `claude-loop capture` prints the loop's pane as plain text, identically on
  Linux and Windows — so a state detector that starts lying can be argued
  about from the actual screen.
- An optional rotating pane cache, `claude_loop.pane_cache_frames`, keeps the
  newest N screens for `claude-loop capture --last <n>`. Only changed screens
  are stored. Off by default.
- `aiball install --service` registers the daemon as a systemd user service, so
  trying aiball and adopting it stay two decisions. `--dry-run` shows the unit,
  `--remove` gives it back.
- `aiball check` names the prerequisites npm can't carry — tmux, python3, a
  Rust toolchain — with the install command for your package manager and
  whether each one blocks aiball or just costs a feature.
- The comment post button gains **post without notifying**: the comment lands
  on the thread but wakes nobody. It suppresses the ping, not the reading.
- `claude-loop bug` packs the logs, `health` verdicts and environment into one
  `.tar.gz` to attach to a report. Built from a whitelist, with tokens and wake
  prompts redacted and pane captures left out unless you ask.
- A coupled ticket is now watched: when its GitHub issue moves, a note lands in
  the thread with a link and the ticket returns to your actionable list.
  Nothing is copied — no upstream text ever overwrites yours. Turn it off with
  `upstream_sync:`, or per repo with `sync:`.
- Upstream coupling can reach GitHub through the `gh` CLI, borrowing its
  credential — so a private repo works with **no token in any aiball config**.
  Default when `gh` is installed and logged in; pin it with
  `upstream_transport:`. A pinned wire never silently falls back.
- The health endpoint reports the daemon's recurring jobs: last run, duration,
  last failure, next due.

### Changed

- **Windows: upgrading needs `install.ps1` run once more.** The packaged `.cmd`
  wrappers are gone; shims written by an earlier installer name deleted files,
  so `aiball`, `claude-loop` and `aiball-mcp` stop resolving from the PATH —
  and the MCP server with them, which reads as an agent losing its aiball tools
  rather than as a broken launcher. `aiball check` now runs each command and
  names the repair.
- The skills aiball ships sit under `skills/`, one directory per skill — the
  layout the skills CLI looks for. Installing them from the repo previously
  found nothing.
- `aiball`, `aiball-mcp` and `claude-loop` are Node entrypoints instead of
  shell launchers duplicated per command and per platform, so each is one
  process. The repo now packs as an npm tarball; nothing is published yet and
  the install paths are unchanged.
- Recurring daemon jobs are declared in one table. A job that fails is recorded
  and skipped instead of taking the daemon down, and one still running when its
  next turn comes is skipped instead of piling up.
- Editing a ticket or comment rewrites it in place, keeping the version as
  originally filed — first and last only. Nothing displays that archive yet; it
  exists so a rewrite can't lose what the ticket first said.

### Fixed

- The loop sees Claude's input box again. It required the two rules framing the
  prompt to be nothing but rule, and Claude Code writes the session label into
  one of them. Releasing the busy indicator and closing a missed turn both
  depended on seeing that box; both were inert.
- A `claude-loop` subcommand that exists is no longer rejected as unknown. The
  gate consulted a second, hand-maintained list of command names.
- Restarting a crew loop no longer turns it back into the lead. Its role and
  ...
  the one a `SIGHUP` triggers — brought it back able to claim from the shared
  pool under the maintainer's identity. `claude-loop check` now shows the role.
  Loops started before this keep their old behaviour until their next start.
- An agent that cannot claim — a crew worker, or an assignment-only project —
  no longer sees the whole shared pool in its backlog and gets woken for
  tickets it can't take. An explicit `@mention` still reaches it, so it never
  becomes unreachable.
- `claude-loop check` and `health` print the loop's state directory, so
  reaching `loop.log` no longer means guessing a hash suffix. `check` also
  resolves the current directory's loop when you don't name one — it used to
  report "(no loop)" from inside a live one.
- The Windows proxy line in `claude-loop check` claimed wake injection went
  through a named pipe. That path moved to the loop socket a while ago.
- A comment can no longer be announced under another ticket's number — enough,
  before, to make an agent reply on the wrong thread. The event it stepped over
  is now left unread instead of silently marked seen.
- A hung `gh` call can no longer block upstream work indefinitely: calls are
  capped and the child killed, surfacing a timeout.
- Search now matches the current text of an edited ticket or comment. It
  indexed the version as originally filed.
- `claude-loop health` no longer reports a dead PTY proxy on a healthy loop. It
  only recognised the Python one, so every loop running the Rust proxy — the
  default — failed that check.
- The bar no longer goes grey while Claude is visibly working. The busy state
  waited on a footer hint Claude only sometimes shows; it now rests on the
  activity line, which stays up for the whole turn.
- `claude-loop` no longer refuses to start once Claude has pruned the session
  it recorded. It checked that the saved id was well-formed, not that it still
  existed, and resuming a pruned one kills the loop with nothing on screen.
- A wake can no longer arrive empty. Events with no body of their own — a
  sub-ticket note, a cross-reference — were rendered as if they were comments.
- A decision wake no longer says "Your plan was ACCEPTED" to the ticket's
  reporter, who is not always the author of the plan.
- Windows: a reload left every earlier loop process alive — several bars
  painting at once, all writing to the same log. Loops now sign a roll-call at
  start, stamped with the machine's boot time so a pid left over from before a
  reboot is never killed.
- Windows: the loop could not tell a greyed suggestion in Claude's prompt from
  real typing. It asked the multiplexer for the cursor in a form that one does
  not understand, and read the reply as nothing on every poll.
- Windows: a status-bar glyph could light but never go out, so the keyboard
  indicator stayed on all session. Clearing one meant assigning an empty value,
  which that multiplexer accepts and ignores; glyphs are now unset instead.
- Windows: the human-typing indicator lit with nobody at the keyboard. A
  fallback detector for proxy-less loops was gated on a Unix socket file
  Windows never creates, so it ran everywhere and fired on the spinner.
- Windows: `claude-loop inspect` and `health` reported a healthy loop as down —
  seven red checks, and zeroed values printed as if measured. Both asked
  whether a socket file existed; Windows connects over loopback instead.
- Windows: with WSL installed, starting a loop could launch the wrong `bash` —
  a terminal flash, then boot forever with an empty log. Git Bash is now
  resolved at every spawn site.
