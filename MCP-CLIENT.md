...
- `ticket_list({ project?, open?, actionable?, claimable? })` — list tickets (filter by project, hide closed). Read-only **exploration** of the backlog — it never claims; use `ticket_claim` to pick up the head. Every row carries a per-consumer **flag bag** computed centrally by `computeTicketFlags` (see `docs/TICKET_LIFECYCLE.md` §5.0.1): `unread`, `actionable`, `claimable`, `is_claim`, `hot`, `backlog_tier` (0/1/2 — 0 not in your backlog, 1 ball in your court, 2 waiting on them), `backlog_cooled_until` (ISO when the row re-surfaces after a recent backlog wake), `gated_by_decision` (true when a `then:plan` / `then:resolved` is pending), `last_actor` + `last_actor_at`. Slice the single list using any of these fields client-side without re-querying.
