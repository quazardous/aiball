...
- **`then: "continue"` with `continue_after_minutes`** — a step keeps the ticket yours and puts it at the top of your backlog (right after events) for `tickets.step_hot_minutes`, 30 by default. `continue_after_minutes` is required with it: `0` when you carry on at once, `N` when the next step waits on something — the ticket stays out of your wakes for N minutes, then leads. N is the soonest a look is worth it, not how long the job takes: when in doubt pick the smaller, since an early look costs one more step and a late one leaves finished work waiting. The wait spends your **wait credit** on the project (60 minutes to start): short of it, the wait is capped to the balance but never under 5 minutes, and coming back on the ticket before the end gives the rest back. Credit is earned by a ticket closed on your accepted resolution (30 when you cited a commit on that ticket, 10 without) or wontfix (5), and by the commits you cite with `commits: ["<sha>", …]` on a reply (1 minute per 20 changed lines, 30 at most, once per commit, read in your checkout, fresh ones only). Every reply answers `wait_credit` with your balance and what the post did to it.
- **`commits` on every comment** — required on a comment (`ticket_reply` without `then: close/reopen`): the SHAs it delivers, or `null` (also `"none"`) when it delivers no commit. A comment without it is refused with the reason (`tickets.require_commits`, on by default). A client that predates the field is warned instead. `/mcp` does not refresh a session's tool schema, a new Claude Code session (restarting the loop) does; until then, end the comment with a line `commits: [<sha>]` or `commits: none`. `N` is at most the project's `tickets.step_after_max_minutes` (120 by default); a longer wait is refused. Refused without `then: "continue"`, and a step without it is refused too.
...
```
[wake fires with the head FIFO event injected: "look #47: TITLE. Triage it, then close the loop: `then: plan` or `resolved`; `then: continue` if the next move is yours, with `continue_after_minutes` when it waits on a job; or a `handback: true` comment if it is someone else's."]
...
```
look #47: which strategy for the migration? Triage it, then close the loop: `then: plan` or `resolved`; `then: continue` if the next move is yours, with `continue_after_minutes` when it waits on a job; or a `handback: true` comment if it is someone else's.
```
