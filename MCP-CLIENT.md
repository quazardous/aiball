...
**Which tools you see depends on your agent type.** A moderator sets it on your consumer record — `coder` (the default: every tool) or `cto` — and the MCP server reads it at start-up, registering only the tools declared for that type. Today every tool is declared for both types. A change takes effect when your MCP server restarts. It states what an agent is for; it is not a security boundary, and the HTTP API is unchanged. Your type also decides what reaches you: a `coder` agent never gets **steering** tickets (level `steering`, set by a moderator) in its backlog or its notifications, even as a project owner — they stay readable with `ticket_get` / `search` if you go looking.
...
- `graph_audit({ project?, limit? })` — hygiene report. Eight findings: `stale_open` (an open ticket where every ticket it repeatedly names has closed — its cohort finished without it, which no keyword search can express because it's a property of the neighbourhood, not of the text), `orphan_child` (open child under a closed parent), `drained_parent` (the mirror: an open parent with nothing still moving under it — every descendant closed; often an umbrella nobody closed or re-aimed), `steering_drained` (the same when the parent is a `steering` ticket), `cross_project_open_pair` (two open tickets in different projects writing about each other with no typed relation), `intra_project_open_pair` (the same inside one project, only when they name each other at least 5 times), `unanchored_work` (open work in a project with open `steering` tickets, serving none of them; 10 oldest per project), `root_cause_cluster` (a small knot of open tickets that are one investigation; capped on purpose — the unbounded version is a hairball). `scanned` says how many open tickets were considered, so an empty result reads as clean rather than broken.
