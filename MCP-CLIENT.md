...
- **`then: "continue"` with `continue_after_minutes`** — a step keeps the ticket yours and puts it at the top of your backlog (right after events) for `tickets.step_hot_minutes`, 30 by default. `continue_after_minutes` is required with it: `0` when you carry on at once, `N` when the next step waits on something — the ticket stays out of your wakes for N minutes, then leads. `N` is at most the project's `tickets.step_after_max_minutes` (120 by default); a longer wait is refused. Refused without `then: "continue"`, and a step without it is refused too.
