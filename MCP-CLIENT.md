...
- **`commits` on every comment** — required on a comment (`ticket_reply` without `then: close/reopen`): the SHAs it delivers, or `null` (also `"none"`) when it delivers no commit. A comment without it is refused with the reason (`tickets.require_commits`, on by default). A client that predates the field is warned instead, until it reconnects. `N` is at most the project's `tickets.step_after_max_minutes` (120 by default); a longer wait is refused. Refused without `then: "continue"`, and a step without it is refused too.
