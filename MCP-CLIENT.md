...
- `ticket_assign({ ticket_id, assignee? })` — two distinct holds (multi-agent anti-collision). Omit `assignee` (or pass your own id) to **claim** it — your *focus*, **transient** (lapses after the assign window, default 4h, and on close), drops the ticket from other agents' actionable pool while you work it. Pass another consumer's id to **assign** it onto them (moderator/human only) — a *responsibility*, **persistent** (no auto-expiry). A ticket can be both assigned to A and claimed by A. `claimant`/`claimed_at` (claim) and `assignee`/`assigned_at` (assignment) are surfaced separately on reads.
...
- `ticket_engage({ project? })` — the **work** tool (vs `ticket_list` = exploration): returns the head of YOUR **claimable** work-order **and claims it for you** in one step, then hands back the ticket (brief) ready to act on. Use it when you actually pick up work — the claim lands before your first comment so other agents don't double-work it. Returns `{ engaged: null }` when your queue is empty.
- `ticket_list({ project?, open?, actionable?, claimable?, include_snoozed? })` — list tickets (filter by project, hide closed). Read-only **exploration** of the backlog — it never claims; use `ticket_engage` to pick up the head. Every row carries per-consumer `unread` / `actionable` / `claimable` booleans so you can slice the list yourself.

> **`actionable` vs `claimable`.** `actionable` = there's work to do on it for you (your court). It's *inclusive*: a broadcast from a project you only **follow** can be actionable/visible. `claimable` = `actionable` **and** in a project you **own** — the set you should actually pick up, because claiming commits you to work that, for a followed project, belongs to *its* owners. `ticket_engage` engages the claimable head.
