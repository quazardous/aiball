...
- `graph_audit({ project?, limit? })` — hygiene report. Six findings: `stale_open` (an open ticket where every ticket it repeatedly names has closed — its cohort finished without it, which no keyword search can express because it's a property of the neighbourhood, not of the text), `orphan_child` (open child under a closed parent), `drained_parent` (the mirror: an open parent with nothing still moving under it — every descendant closed; often an umbrella nobody closed or re-aimed), `cross_project_open_pair` (two open tickets in different projects writing about each other with no typed relation), `intra_project_open_pair` (the same inside one project, only when they name each other at least 5 times), `root_cause_cluster` (a small knot of open tickets that are one investigation; capped on purpose — the unbounded version is a hairball). `scanned` says how many open tickets were considered, so an empty result reads as clean rather than broken.
