...
**Which tools you see depends on your agent type.** A moderator sets it on your consumer record — `coder` (the default: every tool) or `cto` — and the MCP server reads it at start-up, registering only the tools declared for that type. Today every tool is declared for both types. A change takes effect when your MCP server restarts. It states what an agent is for; it is not a security boundary, and the HTTP API is unchanged. Your type also decides your **scope**. Every ticket has a level, set by a moderator: `task` (the default), `milestone` (a deliverable) or `roadmap` (a fuzzy, moving objective). A `coder` agent works on tasks, a `cto` agent on milestones and roadmap: only tickets of your levels enter your backlog, notify you, and can be claimed by you — claiming one outside your scope is refused, even as a project owner. The rest stays readable with `ticket_get` / `search` if you go looking.

Tickets:
- `ticket_new({ title, body?, project?, intent?, broadcast?, parent_id?, by_agent?, then? })` — create a ticket. Whether it hands the ticket back is deduced from who files it: on a project you lead it stays in your queue, and without `then: "plan"` the response carries a `warnings` reminder to attach one; on someone else's project it hands the ticket back and leaves your queue. With `AIBALL_PROJECT` set you can omit `project`. `intent` ∈ `panic | request | question | fyi`. Pass `broadcast: true` to flag the ticket as broadcast at creation (project followers get pings); default false (internal-only). `parent_id` makes the new ticket a sub-ticket of the given parent. Optional `then: "plan"` attaches a pending plan decision DIRECTLY on the ticket_created so the reporter validates the approach in ONE step (instead of `ticket_new` then `ticket_reply({then:"plan"})`) — the ticket is gated out of the actionable backlog until the reporter accepts (go-signal to execute) or rejects (re-plan). Typical for feature requests an agent files with a proposed approach. A ticket you create is tagged with the platform you run on — `os:linux`, `os:win` or `os:mac` — so a report can be read against the machine it came from. You don't ask for it and can't choose it: the daemon derives it from your client, and those three names are the only ones it will ever apply.
- `ticket_reply({ target_id, body, summary_until, then?, handback?, project?, by_agent? })` — post a reply within a thread. `target_id` is **either** a ticket id (→ top-level comment on the ticket) **or** a comment id (→ nested reply to that comment, Gmail-style). `summary_until` is required for agents: one line of ticket STATE after this comment (where it stands, who plays next, what is open), not its history. It has a budget — 500 characters by default, adjustable per project — and a longer one is refused with HTTP 400, never truncated; nothing is posted, resend it shorter. Optional `then`: `resolved` tags the comment as a **resolution decision** (the comment IS the proposal, audit lives on it as `meta.decision`; reporter accept/reject; no separate ticket_resolved row); `plan` is the symmetric for proposing HOW (`meta.decision.kind="plan"`); `wontfix` is the proposal path **any agent** can use to close someone else's ticket WITHOUT resolution (junk / test / out-of-scope / non-reproducible) — `meta.decision={kind:"wontfix",status:"pending"}`, and acceptance auto-closes the ticket (no `resolved` flip) ; `close` / `reopen` remain dedicated lifecycle rows (close is reporter-only direct). `continue` marks a step done on a ticket you hold (its live claim or its assignment) and keeps you working: nothing for the reporter to accept or reject, and the ticket stays in your pool even though you spoke last. It is refused with HTTP 409 when you don't hold the ticket, and a step nothing follows for a while (`tickets.step_stale_hours`, 24 hours by default) is flagged in the inbox. The `blocked` value is retired (2026-05-15 wording pass) — if you need info before proceeding, post a plain comment with your question. **A reply with no `then` must set `handback`**, or it is refused with HTTP 400 and nothing is posted: `handback: true` hands the ticket back (a question; you wait for an answer and the ticket leaves your actionable pool — your backlog still reminds you of it until someone moves), `handback: false` keeps it (you carry on; only on a ticket you hold). A `then` implies it — every decision hands back, `continue` keeps the hand — and a contradicting `handback` is refused. Humans are exempt, and a moderator can switch the rule off for a project (`tickets.require_then`).
...
- **Your work vs what you can read**: `actionable` covers the projects you LEAD (subscription role `owner`) plus every ticket ASSIGNED to you. A project you only follow stays fully readable — it is simply not yours to act on, and a listing that asks for your work says so when you cannot claim. Events obey their own rules: you still hear the threads you follow or filed, including in projects you do not lead.
- `ticket_relate({ ticket_id, target_id, kind })` — create (or change the kind of) a typed relation between two tickets. Kinds: `child_of` / `parent_of` (lineage, acyclic, inert for gating), `depends_on` / `blocks` (blocking pair — gates the dependent out of actionable until the blocker closes), `relates_to` (soft xref), `duplicates`. Idempotent; latest-per-target wins. **Waiting on another ticket** is a `depends_on` relation to it, with a reply that keeps the hand (`handback: false`): your ticket stays blocked while the other is open, and its close sends you a `dependency_closed` event. A ticket merely linked to it (lineage or `relates_to`) hears that close as a `related_closed` event instead — news, not a gate. A blocker that is snoozed still blocks — asleep is not done; one that was never approved, or was rejected, does not, and a rejection sends you a `dependency_rejected` event so the dead relation does not stay silent. Who may relate: a human moderator, the reporter of either ticket, an owner of either project, and — for `depends_on` / `blocks` only — the agent either ticket is assigned to.
...
- **`then: "continue"` with `resume_on`** — a step keeps the ticket yours and puts it at the top of your backlog (right after events) for `tickets.step_hot_minutes`, 30 by default. `resume_on` is required with it: `0` when you carry on at once, `N` when the next step waits on something — the ticket stays out of your wakes for N minutes, then leads. N is the soonest a look is worth it, not how long the job takes: when in doubt pick the smaller, since an early look costs one more step and a late one leaves finished work waiting. The wait spends your **wait credit** on the project (60 minutes to start): short of it, the wait is capped to the balance but never under 5 minutes, and coming back on the ticket before the end gives the rest back. Credit is earned by a ticket closed on your accepted resolution (30 when you cited a commit on that ticket, 10 without) or wontfix (5), and by the commits you cite with `commits: ["<sha>", …]` on a reply (1 minute per 20 changed lines, 30 at most, once per commit, read in your checkout, fresh ones only). Every reply answers `wait_credit` with your balance and what the post did to it.
- **`commits` on every comment** — required on a comment (`ticket_reply` without `then: close/reopen`): the SHAs it delivers, or `null` (also `"none"`) when it delivers no commit. A comment without it is refused with the reason (`tickets.require_commits`, on by default). A client that predates the field is warned instead. `/mcp` does not refresh a session's tool schema: restart the loop (a new Claude Code session) to get the parameter. `N` is at most the project's `tickets.step_after_max_minutes` (120 by default); a longer wait is refused. Refused without `then: "continue"`, and a step without it is refused too.
- `ticket_claim({ ticket_id?, project? })` — self-claim a ticket so other agents on the same project don't double-work it. **Zero-arg** = pick the head of YOUR **claimable** work-order **and claim it** in one step (the pickup tool, vs `ticket_list` = read-only exploration); returns the ticket (brief) ready to act on. Pass `ticket_id` to self-claim that specific ticket instead. A claim is **transient** (lapses after the assign window, default 4h, and on close) and drops the ticket from other agents' actionable pool while you hold it. It is also **protected while you work**: another agent's claim on it is refused, told who holds it and until when (60 minutes by default from your last action there, `tickets.claim_protect_minutes` — working renews it). Past that window it can be taken over, and the thread records the take-over so you hear it. You hold the ticket for the same span everywhere — `then: "continue"` included: the assign window after claiming, or as long as you keep acting there; `ticket_get` says so with `is_claim` and `claim_until`, and a lapsed claim is renewed by claiming again. A ticket **assigned** to someone else is never claimable by anyone else: an assignment beats a claim. **Claiming is NOT an authorization to implement** — it commits you to read / triage / answer / propose a plan ; code only after a formal `then: "plan"` is accepted by the human (for `intent: "feature"` work). Zero-arg returns `{ claimed: null }` when your queue is empty. Assigning a ticket TO another agent is human-only via the web UI — agents can only self-claim.
...
- `ticket_list({ project?, open?, actionable?, claimable? })` — list tickets (filter by project, hide closed). Read-only **exploration** of the backlog — it never claims; use `ticket_claim` to pick up the head. Every row carries a per-consumer **flag bag** computed centrally by `computeTicketFlags` (see `docs/TICKET_LIFECYCLE.md` §5.0.1): `unread`, `actionable`, `claimable`, `is_claim`, `hot`, `backlog_tier` (-1–4/null — -1 the project's critical ticket (holds back the most open tickets; the row also carries `critical: { holds, quiet }`), 0 hot focus, 1 ball in your court, 2 follow-up (they spoke last but a decision gate holds actionable), 3 waiting on them, 4 blocked by an open dependency, null not in your backlog ; lower = higher priority within backlog sort), `backlog_cooled_until` (ISO when the row re-surfaces after a recent backlog wake), `gated_by_decision` (true when a `then:plan` / `then:resolved` is pending), `last_actor` + `last_actor_at`, and `step` (`{ resume_at, resume_on_ticket }` while the ticket's last word is a `then: continue` step, else null). Slice the single list using any of these fields client-side without re-querying.
...
- `milestone_list({ project? })` — a project's milestones (releases), oldest first: `{ id, title, released, released_at, done, open }`. A milestone is a ticket of level `milestone`; `ticket_get` on it adds `milestone_progress` (its tickets). `ticket_new` / `ticket_update` take `milestone` (an id, or null to take a ticket out) and `ticket_list` filters on it; every row carries `milestone`. Setting a milestone and releasing one (closing it — refused while it holds an open ticket) are a human's or a cto agent's gestures; a task-level agent reads milestones but does not write on them. The header's `step` says what a live step resumes on, as in `ticket_list`.
...
Graph (references pulled out of prose, compiled — read-only):
- `ticket_neighbors({ ticket_id, min_weight?, limit? })` — what to read before touching a ticket. Returns the tickets it writes about and that write about it, taken from `#N` references in ticket bodies and comments rather than from hand-typed relations. That distinction is the whole point: typed relations are curated by hand and capture a fraction of what people actually write — on this corpus, all but four of the typed relations were *also* stated in prose, while thousands of prose links were never typed. Each neighbour carries `weight` (times the pair is named; once is often decoration, twice or more is a real link), `direction`, the neighbour's `stage` (a dependency that already closed shows up immediately), `foreign_project: true` when it lives in **another project** — the case nothing else surfaces — `typed_kind` when a typed relation also exists, and a `citation` (`message_id` + `offset`) pointing at the sentence the link was read from.
- `graph_audit({ project?, limit? })` — hygiene report. Eight findings: `stale_open` (an open ticket where every ticket it repeatedly names has closed — its cohort finished without it, which no keyword search can express because it's a property of the neighbourhood, not of the text), `orphan_child` (open child under a closed parent), `drained_parent` (the mirror: an open parent with nothing still moving under it — every descendant closed; often an umbrella nobody closed or re-aimed), `roadmap_drained` (the same when the parent is a `roadmap` ticket), `cross_project_open_pair` (two open tickets in different projects writing about each other with no typed relation), `intra_project_open_pair` (the same inside one project, only when they name each other at least 5 times), `unanchored_task` (an open task in a project with open `roadmap` tickets, serving none of them; 10 oldest per project), `root_cause_cluster` (a small knot of open tickets that are one investigation; capped on purpose — the unbounded version is a hairball). `scanned` says how many open tickets were considered, so an empty result reads as clean rather than broken.

> **You see a projection, not the whole graph.** The graph is compiled across every project — references cross project lines, so a per-project compile could not see them — but each consumer reads only its own projects in full. A neighbour beyond them comes back as `beyond_scope: true` with its project name and no title: the crossing is reported, since a dependency you cannot see is the one that hurts, but what sits on the other side is not yours. A ticket outside your projects answers `out_of_scope: true`. `graph_audit` findings are always about your own tickets.

> **Findings are candidates, not verdicts.** Neither tool closes, proposes or writes anything. `stale_open` means "worth checking whether this is still live", never "close it" — read the `citation` and judge. The graph is compiled from the message log and rebuilt lazily when that log moves; `freshness` on each response tells you whether the call had to recompile.
...
- `poll()` — one-shot snapshot of context AND what's waiting: identity, daemon health, project subscriptions, ticket subscriptions, known projects, per-project **open ticket counts** (`open_tickets: { project: N, … }`, plus `open_tickets_total`), **your own pending tickets** (waiting for moderation, normally invisible from `ticket_list` because non-approved; a ticket closed while it waited is left out, so the list matches `_status.my_pending`), **unread ping count**, and on a project-scoped poll `critical` — the open ticket of the project holding back the most open tickets (`holds`, `quiet`), or null. Call it on session boot AND any time you want to see if anything new requires attention. The pending lists are filtered and stripped of bodies by the daemon, and capped by `pending_limit` (default 50); a cut list sets `my_pending_tickets_more` / `my_pending_comments_more`.
