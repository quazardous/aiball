...
function Write-TrayShortcuts {
    ...
    if (Test-Path $StartupLnk) {
        ...
        Log "removed stale Startup shortcut (autostart is now the scheduled task): $StartupLnk"
    }
}
...
function Stop-AiballTray {
    ...
    try {
        Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*aiball-tray.ps1*' } |
            ForEach-Object {
                try {
                    Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue
                    Log "stopped tray (pid $($_.ProcessId))"
                } catch { }
            }
    } catch { }
}
...
"@
[System.IO.File]::WriteAllText($trayVbsPath, ($trayVbsBody -replace "`r?`n","`r`n"))
...
if ($Service) {
    ...
    if ($NoTray) {
        $taskTarget = $vbsPath        # daemon-launcher.vbs
    ...
    # when the task fires interactively at logon.
