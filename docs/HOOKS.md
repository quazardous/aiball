...
|---|---|---|
| SessionStart | **Used** | It starts the turn machine's idle clock (`SESSION_START`) and reports whether a resume picker is up. It does **not** seal the boot: the boot machine seals only on its own deadline (`DEADLINE_REACHED`), once every boot screen has gone. Emitted slim (`session-start-hook.ts`), same path for resume/clear. Note: the `compact` source is handled but its matcher is **not registered** — a known gap. |
