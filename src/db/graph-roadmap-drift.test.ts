// #2208/#2241 — drift against roadmap objectives. What must hold: a drained parent that is a
// `roadmap` ticket reads as `roadmap_drained` (other parents keep
// `drained_parent`); an open `task` serving no objective is reported only inside a
// project that has an open roadmap ticket, anchoring walks up through closed
...
const ticket = (id: number, project: string, level: "task" | "milestone" | "roadmap" = "task") =>
...
ticket(110, "alpha", "roadmap");            // the objective
...
ticket(115, "alpha", "roadmap");            // an objective whose work all closed
...
test("a drained objective reads as roadmap_drained; an ordinary drained parent keeps drained_parent", () => {
    assert.ok(firstIds("roadmap_drained").includes(115));
    ...
    const f = ofKind("roadmap_drained").find((x) => x.ticket_ids[0] === 115);
...
test("inside a project with an open objective, work that serves none of it is reported", () => {
    const ids = firstIds("unanchored_task");
...
test("anchoring walks up through a closed intermediate", () => {
    assert.equal(firstIds("unanchored_task").includes(114), false);
});
...
test("a project reports its ten oldest unanchored tickets, and says how many there are", () => {
    const delta = ofKind("unanchored_task").filter((f) => f.detail.includes("in delta"));
