...
async function cmdStart(opts: StartOpts): Promise<void> {
    ...
    // both read CL_* vars).
    // #684 david `4hp9j6` — generated artifact, not for hand-editing.
    // To override a value : `CL_FOO=bar claude-loop start <name>` at
    // start time (when shell-env propagation lands), or
    // `claude-loop reload --set CL_FOO=bar` mid-life.
    const envLines = [
