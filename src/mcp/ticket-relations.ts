...
export function registerTicketRelationTools(server: McpServer): void {
    server.registerTool(
        ...
        {
            description:
                "Create (or change the kind of) a typed relation between two tickets, authored on `ticket_id` and pointing at `target_id`. Mirrors the UI add-relation path (#271). Append-only & idempotent: re-posting the same active kind is a no-op; to change a kind, call again with the new one — latest wins WITHIN a relation axis (lineage `child_of`/`parent_of` · gate `depends_on`/`blocks` · `relates_to` · `duplicates`), and relations on DIFFERENT axes to the same target COEXIST (e.g. a `parent_of` lineage AND a `depends_on` gate to the same child both hold). To REMOVE a relation use `ticket_unrelate`.\n\nKinds:\n- `child_of` / `parent_of` — structural lineage (e.g. a bug split out of a parent ticket). Inert for actionable gating. Kept acyclic by the daemon (a cycle-closing edge is rejected, HTTP 409).\n- `depends_on` — this ticket can't proceed until the target closes; gates `actionable_count`. `blocks` is the inverse, posted from the blocker side.\n- `relates_to` — soft cross-reference, no workflow impact.\n- `duplicates` — this ticket duplicates the target (usually paired with a close).\n\nTo say your ticket waits on another one, relate it `depends_on` that ticket: it stays blocked while the other is open, and you get a `dependency_closed` event when it closes. A ticket merely linked to it (lineage or `relates_to`) gets a `related_closed` event at that close instead — news, not a gate. A blocker that is snoozed still blocks — asleep is not done; one that was never approved, or was rejected, does not, and a rejection sends you a `dependency_rejected` event so the dead relation does not stay silent.\n\nPermission (enforced daemon-side): a registered human moderator, the reporter of EITHER ticket, or an owner of either project; for `depends_on` / `blocks`, also the agent either ticket is assigned to. Returns the refreshed `relations[]` for `ticket_id` (and `noop:true` when nothing changed).",
            inputSchema: {
                ticket_id: z
                    .number()
                    .int()
