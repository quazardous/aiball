        ...
        "ticket_reply",
        {
            ...
            inputSchema: {
                ...
                comment_only: z
                    ...
                    .optional()
                    .describe(
                        "#2275 — REQUIRED (true) for a reply with no `then`. A reply that neither carries a `then` nor sets comment_only: true is refused (HTTP 400) and nothing is posted. Set it when the comment concludes nothing: a question, a progress note that proposes nothing, a ticket still pending moderation. If the comment concludes something, attach the matching `then` instead — a status update posted as a plain comment leaves the ticket held by nobody.",
                    ),
                scope: z
                    .enum(MESSAGE_SCOPES)
                    .optional()
