...
export function registerTicketWriteTools(server: McpServer): void {
    ...
    server.registerTool(
        "ticket_reply",
        {
            ...
            inputSchema: {
                ...
                comment_only: z
                    ...
                    .optional()
                    .describe(
                        "#2275 — REQUIRED (true) for a reply with no `then`. A reply that neither carries a `then` nor sets comment_only: true is refused (HTTP 400) and nothing is posted. STRONGLY DISCOURAGED (#2308): a step you finished is `then: \"continue\"`, a step to have validated is `then: \"plan\"`. Keep comment_only for a comment that concludes nothing — a question, a ticket still pending moderation — because a plain comment takes the ticket out of your queue and leaves it held by nobody.",
                    ),
                scope: z
                    .enum(MESSAGE_SCOPES)
                    .optional()
        ...
        async ({ target_id, body, project, by_agent, summary_until, then, comment_only, scope }) => {
            ...
            const res = await client.postMessage({
                ...
                comment_only: kind === "comment_added" ? comment_only : undefined,
