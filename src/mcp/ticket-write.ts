...
const THEN_ON_REPLY: [string, ...string[]] = ["close", "reopen", ...verbsAllowedOn("comment_added"), STEP_VERB];
...
export function registerTicketWriteTools(server: McpServer): void {
    ...
    server.registerTool(
        ...
        {
            description:
                "Post a reply within a ticket thread. `target_id` is either a ticket id (→ top-level comment on the ticket) or a comment id (→ nested reply to that specific comment, Gmail-style). Both produce a comment_added; only parent_id differs.\n\n**`summary_until` is required** (#B.130). It's a one-line **ticket state snapshot AFTER this comment lands** — written so a future agent (or you on next session) can tell from JUST this line where the ticket stands, and whether to open the thread. At most 500 characters by default — longer is refused (HTTP 400), never truncated. NOT a summary of what you just said. See the `summary_until` schema description below for the contract + good/bad examples. The API rejects comment_added without it (HTTP 400).\n\nOptional `then` lets the author tag the comment with a decision intent or chain a unilateral state mutation, in the same call:\n- `then: \"resolved\"` — tag the comment as a *resolution decision* (#B.129). The comment carries `meta.decision={kind:\"resolution\",status:\"pending\"}` and the reporter validates it via accept/reject. Closing the ticket auto-accepts any dangling resolution decisions. The 'decision-on-comment' paradigm: the audit lives in the thread, no separate lifecycle row.\n- `then: \"plan\"` — tag the comment as a *plan proposal* (#B.243), symmetric to `resolved` but for HOW-to rather than DONE. The comment carries `meta.decision={kind:\"plan\",status:\"pending\"}` and the reporter validates the approach before execution. Accepted plan = go-signal (the ticket re-enters actionable so the agent picks it up to execute); pending plan gates actionable just like a pending resolution.\n- `then: \"close\"` — close the ticket. Owner-bypass when posted by the reporter.\n- `then: \"reopen\"` — reopen a closed ticket (resets resolved too).\n- `then: \"continue\"` — mark a step done on a ticket you hold and keep working (on a project you lead, it claims a ticket nobody else holds): nothing to accept or reject, and the ticket stays in your pool even though you spoke last.\n\nUse `then: \"resolved\"` when you've completed the work and propose to close; use `then: \"plan\"` when you've sketched HOW you'll tackle the ticket and want the reporter to validate the approach first. Both post a single comment that also functions as a pending proposal — the reporter sees ONE accept/reject pair under the composer instead of two separate steps.\n\n**Prerequisite — the ticket must already be in `status: \"approved\"`** before you can attach a `then: \"resolved\"` or `then: \"plan\"`. Proposing a decision on a ticket still in moderation `pending` short-circuits the natural flow `pending → approved → working → proposed → accepted/rejected` — the reporter hasn't yet validated that the ticket is even actionable. The API rejects with HTTP 409 and the message `cannot propose <kind> on a ticket in status \"<status>\"`. If you've already done the work and the ticket is still pending, post a plain comment_added with your update + ping the reporter to moderate ; once the ticket is approved, then post your `then: resolved`. `ticket_get` surfaces a `decision_proposable: false` flag on pending tickets so you can check before posting.\n\nIf you need more info before you can proceed, just post a plain comment with your question — there is no agent→human \"blocked\" signal anymore (it induced misuse where agents temporized with blocked instead of asking; the conversational comment IS the right primitive). A reply with no `then` must say whether it hands the ticket back (#2331): `handback: true` (a question, you wait for an answer; the ticket leaves your actionable pool, and your backlog still reminds you of it until someone moves) or `handback: false` (you keep working on it, only on a ticket you hold). A `then` implies it: every decision hands back, `then: \"continue\"` keeps the hand; a `handback` that contradicts the `then` is refused.",
            inputSchema: {
                target_id: z
                    .number()
                    .int()
                ...
                summary_until: z
                    ...
                    .describe(
                        [
                            ...
                            "",
                            "Latest-wins: only the most recent summary_until is canonical. It is the ticket's STATE, not its history — a reader who needs the history opens the thread, so do not replay the thread here. **Budget: 500 characters by default** (a moderator may set another per project); a longer one is REFUSED with HTTP 400, never truncated, and nothing is posted — resend it shorter. Humans are exempted from both the requirement and the budget. Mandatory for agents — the API rejects comment_added without it (HTTP 400).",
                            "",
                            "Light markdown is OK and rendered in the web-UI tldr banner (`**bold**`, `` `code` ``, lists, links). Keep it light — it's a compact one-liner, not a document.",
                        ].join("\n"),
                    ),
                then: z
                    .enum(THEN_ON_REPLY)
                ...
                resume_on: z
                    ...
                    .optional()
                    .describe(
                        "#2765 — REQUIRED with `then: \"continue\"`, refused with anything else: what you resume on, whichever comes first. `{ timer: 0 }` = you carry on at once. `{ timer: N }` = the next step waits on a job (a build, a test box, a deploy): the ticket stays out of your wakes N minutes — the soonest a look is worth it, not how long the job takes (a job of ~20 min: 10). A job you start in the background wakes you when it ends: then the timer is only a fallback, for a job that hangs. `{ ticket: N }` = resume when ticket N moves (a reply, a decision, a close), e.g. the ticket this one is queued behind. Both = whichever comes first. The timer spends your wait credit (short of it, capped to your balance, never under 5 min; coming back early gives the rest back); waiting on a ticket costs nothing. `timer` is at most the project's `tickets.step_after_max_minutes` (120 by default). Then the ticket leads your backlog.",
                    ),
                commits: z
        ...
        async ({ target_id, body, project, by_agent, summary_until, then, handback, resume_on, commits, scope }) => {
            ...
            const res = await client.postMessage({
                ...
                // #2331 — the explicit handback of a reply with no `then`.
                ...
                step_after_minutes: resume_on?.timer,
                // #2765 — resume when that ticket moves; with a timer, whichever comes first.
                step_resume_on_ticket: resume_on?.ticket,
    ...
    server.registerTool(
        ...
        {
            description:
                "Self-claim a ticket so other agents on the same project don't double-work it. Zero-arg = pick the head of YOUR CLAIMABLE work-order (same gate + order as `ticket_list({claimable: true})` — priority desc, then hot, then oldest) AND claim it in one step. Pass `ticket_id` to self-claim that specific ticket instead. Claimable = actionable AND in a project you OWN, so a bare `ticket_claim()` never claims a follower-broadcast from a project you only follow (that work belongs to that project's owners, #432). A live claim drops the ticket out of every OTHER agent's actionable pool while you work it (anti-collision, #418), and the claim lands BEFORE your first comment so your intent shows immediately. Your claim is also PROTECTED while you work: another agent claiming it is refused, told who holds it and until when (`tickets.claim_protect_minutes`, 60 by default, counted from your last action there — working renews it). Past that window someone may take it over, and the thread records the take-over. Claiming a ticket ASSIGNED to someone else is always refused: an assignment beats a claim. Use THIS — not `ticket_list` — when you actually pick up work: `ticket_list` is the EXPLORATION/backlog tool (read-only, never claims). **Claiming is NOT an authorization to implement.** It commits you to read / triage / answer / propose a plan. For `intent: \"feature\"` work or an umbrella ticket with sub-tickets, code only after a formal `then: \"plan\"` is accepted by the human ; claiming a parent never cascades the go to its children (#669). Zero-arg returns `{ claimed: null }` when your queue is empty. Idempotent: re-claiming the head you already hold just refreshes the claim window. Assigning a ticket TO another agent is human-only via the web UI — agents can only self-claim.",
            inputSchema: {
                ticket_id: z
                    .number()
                    .int()
