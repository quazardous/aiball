...
export function registerTicketReadTools(server: McpServer): void {
    ...
    server.registerTool(
        ...
        {
            description:
                "Hygiene report over the ticket corpus, computed from the compiled reference graph. Eight kinds of finding: `stale_open` (an open ticket where every ticket it repeatedly names has closed — its cohort finished without it, which no keyword search can express because it is a property of the neighbourhood, not of the text), `orphan_child` (open child under a closed parent), `drained_parent` (the mirror: an OPEN parent with nothing still moving under it — every descendant, all the way down, is closed; often an umbrella whose work finished and that nobody closed or re-aimed), `steering_drained` (the same when the parent is a `steering` ticket: an objective with nothing moving under it), `cross_project_open_pair` (two OPEN tickets in different projects writing about each other with no typed relation — invisible everywhere else), `intra_project_open_pair` (the same silence INSIDE one project, reported only when the two name each other at least 5 times: across projects it is two teams ignoring each other, here it is someone not linking their own threads), `unanchored_work` (open `work` in a project that has open `steering` tickets, serving none of them — asked only there, capped at the 10 oldest per project with the total), and `root_cause_cluster` (a small knot of open tickets that are all one investigation; deliberately capped, since the unbounded version is a 153-ticket hairball that tells nobody anything).\n\n**Every finding is a CANDIDATE, never a verdict.** This tool closes nothing, proposes nothing and writes nothing — it hands you a `citation` (`message_id` + `offset`) so you can read the sentence and judge. Treat `stale_open`, `drained_parent`, `steering_drained` and `unanchored_work` as 'worth checking whether this is still live', not as 'close it'. `scanned` reports how many open tickets were considered, so an empty result reads as clean rather than broken, and `freshness` says whether this call had to recompile. Findings are only ever ABOUT tickets in your own projects: the audit reasons inside your projection, and a link leaving it is reported by `ticket_neighbors` as a boundary rather than half-redacted here.",
            inputSchema: {
